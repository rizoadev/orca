...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    'let pendingReasoningPart = null;',
    'let reasoningPartFlushTimer = null;',
    'let lastReasoningPartPostAt = 0;',
    '',
    ...
    '}',
    '',
    'async function flushPendingReasoningPart() {',
    '  if (reasoningPartFlushTimer) {',
    '    clearTimeout(reasoningPartFlushTimer);',
    '    reasoningPartFlushTimer = null;',
    '  }',
    '  const pending = pendingReasoningPart;',
    '  pendingReasoningPart = null;',
    '  if (!pending) return;',
    '  lastReasoningPartPostAt = Date.now();',
    '  await post("MessagePart", {',
    '    role: "reasoning",',
    '    text: capMessagePartText(pending.text),',
    '    messageID: pending.messageID,',
    ...
    '}',
    '',
    'function queueReasoningPart(part) {',
    '  pendingReasoningPart = part;',
    '  const sinceLastPost = Date.now() - lastReasoningPartPostAt;',
    '  if (sinceLastPost >= MESSAGE_PART_THROTTLE_MS) {',
    '    void flushPendingReasoningPart();',
    '    return;',
    '  }',
    '  if (!reasoningPartFlushTimer) {',
    '    reasoningPartFlushTimer = setTimeout(() => {',
    '      void flushPendingReasoningPart();',
    '    }, MESSAGE_PART_THROTTLE_MS - sinceLastPost);',
    '    if (reasoningPartFlushTimer.unref) reasoningPartFlushTimer.unref();',
    '  }',
    '}',
    '',
    ...
    '      // text; a `reasoning` part carries the chain-of-thought we surface',
    '      // as a thinking-aside in the chatbox. Skip non-text/reasoning parts',
    '      // (tool, file, …) so we only forward what the dashboard renders.',
    '      // Role for text parts came from the earlier message.updated event;',
    '      // if we never saw one (e.g. plugin loaded mid-turn) the role is',
    '      // unknown, and mislabeling the part — a user prompt displayed as',
    '      // the assistant reply, or vice versa — is worse than silently',
    '      // dropping a single in-flight text chunk. The next message.updated',
    '      // event will re-seed the role cache, so subsequent parts in the',
    '      // same session flow normally.',
    '      const part = event.properties && event.properties.part;',
    '      if (!part || !part.text) return;',
    '      if (part.type === "reasoning") {',
    '        queueReasoningPart({ text: part.text, messageID: part.messageID, sessionID });',
    '        return;',
    '      }',
    ...
    '      // Reasoning parts drain in lockstep so the chatbox collapses the',
    '      // thinking-aside into its final form alongside the assistant reply.',
    '      await flushPendingAssistantPart();',
    '      await flushPendingReasoningPart();',
