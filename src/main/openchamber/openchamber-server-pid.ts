/**
 * Server-child pid bookkeeping for the OpenChamber manager.
 *
...
/** Best-effort check that a pid is really our server (Linux /proc only). */
function pidIsOpenChamber(pid: number, entrypoint: string): boolean {
  if (process.platform !== 'linux') {
    return false
  }
  try {
    const cmdline = readFileSync(`/proc/${pid}/cmdline`, 'utf8')
    return cmdline.includes('openchamber') || cmdline.includes(entrypoint)
  } catch {
    return false
  }
}
...
export async function reapOrphanServer(dataDir: string, key: string): Promise<void> {
  try {
    ...
    if (isPidAlive(recorded.pid) && pidIsOpenChamber(recorded.pid, recorded.entrypoint)) {
...
export function recordServerPid(
  dataDir: string,
  key: string,
  pid: number,
  entrypoint: string
  ...
  try {
    ...
    writeFileSync(pidFilePath(dataDir, key), JSON.stringify({ pid, entrypoint }))
  } catch {
