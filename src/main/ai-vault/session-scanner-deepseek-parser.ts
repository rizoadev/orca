...
import { extractMessageText } from './session-scanner-text-normalization'
...
export async function parseDeepSeekSessionFile(
  ...
  // Why: DSH stores `session.jsonl.zstd` as concatenated per-event frames, so
  // the raw bytes must be read (not streamed as lines) before decompression.
  const content = decompressConcatenatedZstdFrames(await readFile(file.path)).toString('utf8')
...
function decompressConcatenatedZstdFrames(input: Buffer): Buffer {
  ...
  while (offset < input.length) {
    ...
    if (
      ...
      consumedBytes <= 0 ||
      offset + consumedBytes > input.length
    ) {
